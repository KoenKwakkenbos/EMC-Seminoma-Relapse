...
import config_custom_10 as config
...
for i, (train_index, val_index) in enumerate(skf.split(patient_data.index, patient_data['Event'])):
    ...
    wandb.init(
        ...
        project="Custom_Fully_Supervised_Survival",
    ...
    train_gen = get_datagenerator(tiles_train, labels_train[0], labels_train[1], labels_train[2], config.train_settings['batch_size'], train=True, imagenet=False)
    val_gen = get_datagenerator(tiles_val, labels_val[0], labels_val[1], labels_val[2], config.train_settings['batch_size'], train=False, imagenet=False)

    model = create_cnn_model(input_shape=(config.model_settings['tile_size'], config.model_settings['tile_size'], 3), num_clinical_features=0)
    print(model.summary())

    trainer = TrainAndEvaluateModel(
        model=model,
        model_dir=Path(f"Custom_Fully_Supervised_Survival_fold_{i+1}"),
